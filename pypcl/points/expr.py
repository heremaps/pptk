...
class expression(object):
	...
	def __init__(self):
		raise RuntimeError(\
			'expression class not meant for instantiation')
	def __len__(self):
	...
	def _check_operands():
		raise RuntimeError(\
			'method meant for overriding in subclass')
	...
	def __iter__(self):
		...
		num_items = len(self)
		chunk_size = max(_min_chunk_size,min(_max_chunk_size,\
	...
	def __getitem__(self,key):
		...
		if isinstance(key,expression):
			return index_op(self,key,slice(None,None,None))
				...
				isinstance(key[0],expression) and \
				isinstance(key[1],slice) \
			or \
				isinstance(key[0],slice) and \
				isinstance(key[1],expression) \
			or \
				isinstance(key[0],slice) and \
				isinstance(key[1],slice)):
...
class unary_op(expression):
	...
	def __repr__(self):
		return 'Expression %s %s (%d elements)'%\
			(self.op,type(self),len(self))+\
			expression.__repr__(self)
	@staticmethod
	def _check_operands(operand):
		pass
	def _evaluate_chunk(self,index,size,use_cache=False):
		operandlist = expression._evaluate_chunk(\
			self.operand,index,size,use_cache)
...
class binary_op(expression):
	...
	def _check_operands(left,right):
		...
		if len(left)!=len(right) and \
			...
			raise ValueError('attempting to operate on incompatible \
				lengths %d and %d' %(len(left),len(right)))
	def _evaluate_chunk(self,index,size,use_cache=False):
		leftlist = []
		if len(self.left)==1:
			leftlist = expression._evaluate_chunk(\
				self.left,0,1,use_cache)
		else:
			leftlist = expression._evaluate_chunk(\
				self.left,index,size,use_cache)
		rightlist = []
		if len(self.right)==1:
			rightlist = expression._evaluate_chunk(\
				self.right,0,1,use_cache)
		else:
			rightlist = expression._evaluate_chunk(\
				self.right,index,size,use_cache)
...
class list_expression(expression):
	...
	def __repr__(self):
		return 'Expression %s (%d elements)'%\
			(type(self),len(self))+\
			expression.__repr__(self)
	@staticmethod
	def _check_operands():
...
class	index_op(expression):
	...
	def _check_operands(src,row_sel,col_sel):
		...
		sel_len = None
		if isinstance(row_sel,expression) and \
			isinstance(col_sel,slice):
			sel_len = len(row_sel)
		elif isinstance(row_sel,slice) and \
			isinstance(col_sel,expression):
			sel_len = len(col_sel)
		elif isinstance(row_sel,slice) and \
			isinstance(col_sel,slice):
			sel_len = 1
			...
			raise TypeError(\
				'Incompatible row + column selector types (%s,%s)' % \
				(type(row_sel),type(col_sel)))
	...
	def _evaluate_chunk(self,index,size,use_cache=False):
		srclist = []
		if len(self.src)==1:
			srclist = expression._evaluate_chunk(\
				self.src,0,1,use_cache)
		else:
			srclist = expression._evaluate_chunk(\
				self.src,index,size,use_cache)
		selector = [None,None]
		if isinstance(self.row_sel,slice):
			selector[0] = self.row_sel
		elif len(self.row_sel)==1:
			selector[0] = expression._evaluate_chunk(\
				self.row_sel,0,1,use_cache)
		else:
			selector[0] = expression._evaluate_chunk(\
				self.row_sel,index,size,use_cache)
		if isinstance(self.col_sel,slice):
			selector[1] = self.col_sel
		elif len(self.col_sel)==1:
			selector[1] = expression._evaluate_chunk(\
				self.col_sel,0,1,use_cache)
		else:
			selector[1] = expression._evaluate_chunk(\
				self.col_sel,index,size,use_cache)
