...
public:
	struct Node {
		...
		bool is_leaf;
		Node * children[8];	// index given by 3-bit code: xyz, where z is least significant bit
	};
	...
	struct CameraFrustum {
		...
		static void setupCameraFrustum(
			CameraFrustum & frustum, const Camera & camera,
			const float z_near, const float vfov, const float aspect_ratio)
		{
	...
	Octree() : 
		_max_leaf_size(64), _num_points(0), _root(NULL), _ptr_point_xyz(NULL), _cube_size(0.0f) {
	...
	unsigned int countInnerNodes() {
		return countNodesHelper(_root, false);
	}
		...
		unsigned int max_leaf_size = 64)
	{
		...
		float cube_center[3] = {
			0.5f * (box.x(0) + box.x(1)),
			0.5f * (box.y(0) + box.y(1)),
			0.5f * (box.z(0) + box.z(1))};
		float cube_size = (std::max)(
			box.x(1) - box.x(0), (std::max)(
			box.y(1) - box.y(0),
			box.z(1) - box.z(0)));
		float cube_corner[3] = {
			cube_center[0] - 0.5f * cube_size,
		...
		// begin recursion
		_root = buildTreeHelper(
			&_indices[0], &_labels[0], _num_points, _lower_left_corner, _cube_size);
	...
	void getIndices (
		...
		const Camera & camera,
		float vfov, float z_near, unsigned int width, unsigned int height, float fudge_factor = 0.25f) const {
		...
		getIndicesHelper(indices, _root, frustum, _lower_left_corner, _cube_size, eps, PERSPECTIVE, fudge_factor);
	}
	void getIndicesOrtho (
		std::vector<unsigned int> & indices, 
		const Camera & camera,
		float image_r, float image_t, unsigned int height, float fudge_factor = 0.25f) const {
		...
		indices.reserve(_num_points);
		getIndicesHelper(indices, _root, frustum, _lower_left_corner, _cube_size, eps, ORTHOGRAPHIC, fudge_factor);
	}
	void getClickIndices (
		std::vector<unsigned int> & indices,
		...
		const ProjectionMode projection_mode) const
	{
		...
		if (projection_mode == PERSPECTIVE)
			CameraFrustum::setupCameraFrustum(frustum, camera, -near_clip, vfov, screen_width / screen_height);
		else
			CameraFrustum::setupOrthoCamera(frustum, camera, vfov, screen_width / screen_height);
		...
		getClickIndicesHelper(indices, d_min,
			_root, _lower_left_corner, _cube_size, 
		...
		const ProjectionMode projection_mode) const
	{
		...
		getClickIndicesBruteHelper(min_idx, d_min, 
			&points_xyz[0], _num_points, 
			click_pos, click_radius, frustum, projection_mode);
...
private:
	static void traversalOrder(
		unsigned int (&nodeIndices)[8],
		const float (&view)[3])
	{
		int b[3][2];
	...
	static float pointToPointDistanceSquared(const float (&x)[2], const float (&y)[2]) {
		float distance = 0.0f;
	...
	static float boxToPointDistance(
		const float (&box_center)[2],
	...
	static void projectNode(
		float & v_min,
		float & v_max,
		const float (&node_corner)[3],
	...
	static void getClickIndicesBruteHelper(
		...
		const float * points, 
		const int count, 
		const float (&click_pos)[2],
		const float click_radius, 
		...
		const ProjectionMode & projection_mode)
	{
		...
		for (int i = 0; i < count; i++) {
			...
			float point_proj[2] = {
				dotProduct(xyz_eye, frustum.right), 
		...
		const ProjectionMode projection_mode = PERSPECTIVE) const
	{
		...
		// compute conservative bound around projection of node onto z = -1 image plane
		float bound_center[2];
		float bound_size[2];
		if (projection_mode == PERSPECTIVE)
			boundProjectedAABB(bound_center, bound_size, node_center_ruv, node_size, frustum.z_near);
		else
			boundOrthoProjectedAABB(bound_center, bound_size, node_center_ruv, node_size);
		...
		if (node->is_leaf) {
			...
			int min_idx;
			getClickIndicesBruteHelper(min_idx, d_min, 
				&point_xyz[3 * node->point_index], node->point_count, 
				click_pos, click_radius, frustum, projection_mode);
			if (min_idx != -1) {
			...
			if (projection_mode == PERSPECTIVE) {
				float node_center_eye[3] = {
					node_center[0] - frustum.eye[0],
			...
			for (int i = 0; i < 8; i++) {
				...
				getClickIndicesHelper(indices, d_min,
					child_node, child_node_corner, 0.5f * node_size, 
					click_pos, click_radius, frustum, projection_mode);
			}
			if (!indices.empty())
		...
		unsigned int bit = 2) 
	{
		...
		if (bit == 0) {
			...
			partitionXYZ(child_counts + offset, indices + left, labels + left, count - left, bit - 1);
		}
	}
		...
		const float cube_size)
	{
		...
		if (count == 0) {
			...
			for (unsigned int i = 0; i < count; i++) {
				labels[i] = 0;
				for (unsigned int dim = 0; dim < 3; dim++)
					labels[i] |= (point_xyz[3 * indices[i] + dim] > cube_center[dim] ? 1 : 0) << (2 - dim);
			}
			...
			for (unsigned int i = 0; i < 8; i++) {
				...
				node->children[i] = buildTreeHelper(
					ptr_indices, ptr_labels, child_counts[i], child_corner, 0.5f * cube_size);
	...
	static IntersectResult intersectBoxes2D(
		const float (&box_1_center)[2], const float (&box_1_size)[2],
		const float (&box_2_center)[2], const float (&box_2_size)[2])
	{	
		if (
			box_1_center[0] + 0.5f * box_1_size[0] < box_2_center[0] - 0.5f * box_2_size[0] || 
			box_1_center[0] - 0.5f * box_1_size[0] > box_2_center[0] + 0.5f * box_2_size[0] || 
			box_1_center[1] + 0.5f * box_1_size[1] < box_2_center[1] - 0.5f * box_2_size[1] || 
			box_1_center[1] - 0.5f * box_1_size[1] > box_2_center[1] + 0.5f * box_2_size[1])
			return OUTSIDE;
		else if (
			box_1_center[0] + 0.5f * box_1_size[0] < box_2_center[0] + 0.5f * box_2_size[0] && 
	...
	{
		// note: does not work
		float cube_bounds[3][2] = {
			{cube_corner[0], cube_corner[0] + cube_size},
		...
		for (int k = 0; k < 2; k++) {
			for (int j = 0; j < 2; j++) {
				for (int i = 0; i < 2; i++) {
					...
					ruv[0] /= -ruv[2];
					ruv[1] /= -ruv[2];
					if (-ruv[2] > 0.0f &&	// note: view vector is opposite of view direction
	...
	void getIndicesHelper(
		...
		const float fudge_factor = 0.25f,	// factor by which projected area is reduced
		const IntersectResult parent_intersect_result = UNCERTAIN) const 
	{
		...
		float bound_size[2];
		if (projection_mode == PERSPECTIVE)
			boundProjectedAABB(bound_center, bound_size, ruv, cube_size, frustum.z_near);
		else
		...
		if (parent_intersect_result == INSIDE)
			// if parent was entirely in frustum, then this node is also entirely in frustum
			intersect_result = INSIDE;
		else {	// parent_intersect_result == UNCERTAIN
			if (projection_mode == PERSPECTIVE && -ruv[2] + 0.5f * cube_size * sqrtf(3.0f) < 0.0f)
				return;
			float image_center[2] = {0.0f, 0.0f};
			float image_size[2] = {2.0f * frustum.image_r, 2.0f * frustum.image_t};
			intersect_result = intersectBoxes2D(bound_center, bound_size, image_center, image_size);
		}
#if 0
			...
			for (int k = 0; k < 2; k++) {
				for (int j = 0; j < 2; j++) {
					for (int i = 0; i < 2; i++) {
						...
						getIndicesHelper(indices, node->children[node_index],
							frustum, child_corner, 0.5f * cube_size, eps, projection_mode, fudge_factor, intersect_result);
					}
				}
			}
		}
	...
	unsigned int countNodesHelper(const Node * node, bool count_leaf = true) {
		if (!node) return 0;
		else if (node->is_leaf) return count_leaf ? 1 : 0;
		else {
			unsigned int count = 0;
			for (int i = 0; i < 8; i++)
