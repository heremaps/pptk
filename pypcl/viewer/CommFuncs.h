...
namespace comm {
	template <typename T> struct TypeCode {static const unsigned char value = 0;};
	template <> struct TypeCode<char> {static const unsigned char value = 1;};
	template <> struct TypeCode<float> {static const unsigned char value = 2;};
	template <> struct TypeCode<int> {static const unsigned char value = 3;};
	template <> struct TypeCode<unsigned int> {static const unsigned char value = 4;};

	inline void receiveBytes(
		char * destination,
		const qint64 bytesExpected,
	...
	template <typename T>
	void sendMatrix(
		const T * source,	// in column major order
		const quint64 numRows,
		const quint64 numCols,
		QTcpSocket * clientConnection)
	{
		// send data type
		unsigned char dataType = TypeCode<T>::value;
	...
	template <typename T>
	void sendMultiDimArray(
		const T * source,
		const std::vector<quint64> & dims,
		QTcpSocket * clientConnection)
	{
		// send data type
		unsigned char dataType = TypeCode<T>::value;
