...
public:
	...
	void loadPoints(std::vector<float> & positions) {
		...
		glBufferData(GL_ARRAY_BUFFER, _positions.size() / 3 * sizeof(float), NULL, GL_DYNAMIC_DRAW);
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, _num_points * sizeof(unsigned int), NULL, GL_DYNAMIC_DRAW);
	...
	void draw(const unsigned int * indices, const unsigned int num_points, const QtCamera & camera, const SelectionBox * box = NULL) {
		if (_num_points == 0)
			return;
		...
		_program.bind();
		_program.setUniformValue("width", (float)_window->devicePixelRatio() * _window->width());
		_program.setUniformValue("height",(float)_window->devicePixelRatio() * _window->height());
		_program.setUniformValue("point_size", _point_size);
		_program.setUniformValue("mvpMatrix", camera.computeMVPMatrix(_full_box));
		_program.setUniformValue("box_min", box ? box->getBox().topLeft() : QPointF());		// topLeft in Qt is bottom left in NDC
		_program.setUniformValue("box_max", box ? box->getBox().bottomRight() : QPointF());	// top right in NDC
		...
		_program.setUniformValue("box_select_mode", box ? box->getType() : SelectionBox::NONE);
	...
	void queryLOD(std::vector<unsigned int> & indices, const QtCamera & camera, const float fudge_factor = 1.0f) {
		float min_z_near = 0.1f;
		if (camera.getProjectionMode() == QtCamera::PERSPECTIVE)
			_octree.getIndices(indices, camera, camera.getVerticalFOV(), -min_z_near, _window->width(), _window->height(), fudge_factor);
		else {
			float t = camera.getCameraDistance() * tan(0.5f * camera.getVerticalFOV());
			float r = (float)_window->width() / _window->height() * t;
			_octree.getIndicesOrtho(indices, camera, r, t, _window->height(), fudge_factor);
		}
	}
	...
	void initColors() {
		...
		if (use_color_map) {
			...
			glTexImage1D(GL_TEXTURE_1D, 0, 4, (int)_color_map.size() / 4, 0, GL_RGBA, GL_FLOAT, (GLvoid *)&_color_map[0]);
			glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_MAX_LEVEL, 0);	// not sure why this is needed, but it is
		} else {
			// use color map that always returns white
			float white[4] = {1.0f, 1.0f, 1.0f, 1.0f};
			glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA, 1, 0, GL_RGBA, GL_FLOAT, (GLvoid *)white);
		}
		...
		if (!broadcast_attr) {
			...
			glBufferData(GL_ARRAY_BUFFER, sizeof(float) * attr.size(), (GLvoid *)&attr[0], GL_STATIC_DRAW);
		}
		_context->doneCurrent();
		if (_color_map_auto) {
	...
	void setColorMapScale(float scale_min, float scale_max) {
		...
		if (scale_min >= scale_max) {
			_color_map_auto = true;
			// automatically set [_color_map_min, _color_map_max] according to the attribute type
			int curr_attr_idx = (int)_attributes.currentIndex();
			bool use_color_map = _attributes.dim(curr_attr_idx) == 1;
			bool broadcast_attr = _attributes.size(curr_attr_idx) == 1;
			const std::vector<float> & attr = _attributes[curr_attr_idx];
	...
	{
		if (box.getType() == SelectionBox::NONE)
			return;
	...
	void queryNearPoint(std::vector<unsigned int> & indices, const QPointF & point, const QtCamera & camera)
	{
		Octree::ProjectionMode projection_mode = camera.getProjectionMode() == QtCamera::PERSPECTIVE ? Octree::PERSPECTIVE : Octree::ORTHOGRAPHIC;
		_octree.getClickIndices(indices,
			point.x(), _window->height() - point.y() - 1.0f, 5.0f,
			_window->width(), _window->height(), camera.getVerticalFOV(), 0.1f, camera, projection_mode);
	}

	void selectNearPoint(const QPointF & point, const QtCamera & camera, bool deselect = false)
	{
		std::vector<unsigned int> new_indices;
	...
	{
		// returns indices into original array of points (prior to reshuffling by octree)
		indices.reserve(_selected_ids.size());
		indices.clear();
		for (std::size_t i = 0; i < _selected_ids.size(); i++)
			if (_selected_ids[i] < _num_points) indices.push_back(_octree.getIndices()[_selected_ids[i]]);
			else break;
	}
	...
	QVector3D computeSelectionCentroid() {
		...
		if (num_selected == 0) {
			return QVector3D(
				0.5f * (_full_box.x(0) + _full_box.x(1)),
	...
	std::size_t getNumSelected() const {return countSelected(_selected_ids, (unsigned int)_num_points);}
	std::size_t getNumAttributes() const {return _attributes.numAttributes();}
	std::size_t getCurrentAttributeIndex() const {return _attributes.currentIndex();}
	const PointAttributes & getAttributes() const {return _attributes;}
	const std::vector<float> & getPositions() const {return _positions;}
	const std::vector<unsigned int> & getSelectedIds() const {return _selected_ids;}
...
private:
	...
	static void mergeIndices(std::vector<unsigned int> & x, const std::vector<unsigned int> & y, bool xor_merge = false) {
		...
		for (std::size_t i = 0; i < x.size(); i++) {
			while(y_idx < y.size() && y[y_idx] < x[i])
				temp.push_back(y[y_idx++]);
			if (y_idx == y.size()) {
	...
	static std::size_t countSelected(const std::vector<unsigned int> & x, unsigned int y) {
		...
		// find number of items in _selected_ids (sorted) that is less than _num_points
		if (x.empty()) return 0;
		std::size_t a = 0;
	...
	std::vector<unsigned int> _selected_ids;	// keeps track of user selected points
