#ifndef CAMERADOLLY_H
#define CAMERADOLLY_H
#include <QVector3D>
#include <QtGlobal>
#include <vector>
#include "timer.h"
...
public:
	 ...
	 */
	CameraDolly(const std::vector<float> &ts, const std::vector<CameraPose> &poses,
				InterpolationType interp = LINEAR, bool repeat = false) :
		_ts(ts), _poses(poses), _interp_type(interp), _repeat(repeat), _active(false)
	{
		check_and_init();
		compute_splines();
	}
	...
	void start () {
		...
		_current_time = _start_time;	// this shouldn't be necessary, but just in case
	}
	void stop() {
		_active = false;
	}
	...
	CameraPose getPose(float t) const {
		CameraPose pose;
		QVector3D look_at(_look_at_x->eval(t), _look_at_y->eval(t), _look_at_z->eval(t));
