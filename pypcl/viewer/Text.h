...
/* following text rendering code adapted from Text.h and Text.cpp in mifit project: https://code.google.com/p/mifit/ */
...
public:
	Text(QWindow * window, QOpenGLContext * context, const QFont & f) :
		...
		font(f), fontMetrics(f), pixelFont(f), pixelFontMetrics(f), xOffset(1), yOffset(1)
	{
		_context->makeCurrent(_window);
		initializeOpenGLFunctions();
		_context->doneCurrent();

		// font sizes in units of pixels (I don't really know how this works... this is a hack)
		if (_window->devicePixelRatio() != 1.0)
			pixelFont.setPixelSize(qRound(_window->devicePixelRatio() * font.pointSize()));
