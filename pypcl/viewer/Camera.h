...
public:
	...
	static void computeRightVector(
		float (&v)[3],
		const float theta,
		const float phi)
	{
...
private:
	static void computeCameraPosition(
		float (&p)[3],
	...
	static void computeCameraFrame(
		float (&x)[3],
		float (&y)[3],
		float (&z)[3],
		const float theta,
	...
	static void computeCameraMatrix(
		float (&m)[16],
		float (&lookAt)[3],
		float theta,
		float phi,
