...
public:
	...
	T eval(T t) const {
		...
		if (idx == 0)
			...
			// at this point, we know ts.size() > 1 and  0 < idx < _ts.size()
			float dt = (t - _ts[idx - 1]) / (_ts[idx] - _ts[idx - 1]);
			float c[4] = {_vs[idx - 1], _coeffs_1[idx - 1], _coeffs_2[idx - 1], _vs[idx]};
...
private:
	void setupLinearSystem (SpMat & A, Eigen::VectorXf & b) {
		...
		for (int i = 0; i < num_knots; i++) {
			...
			b(2 * i) = -3.0f * _vs[i + 1] * delta_inv[i + 1] + -3.0f * _vs[i + 1] * delta_inv[i];
			...
			b(2 * i + 1) = 6.0f * _vs[i + 1] * delta_inv_2[i + 1] - 6.0f * _vs[i + 1] * delta_inv_2[i];
		}
		if (_boundary_behavior == PERIODIC) {
			...
			b(n - 2) = 3.0f * _vs.back() * delta_inv.back() + 3.0f * _vs.front() * delta_inv.front();
			...
			b(n - 1) = 6.0f * _vs.back() * delta_inv_2.back() - 6.0f * _vs.front() * delta_inv_2.front();
