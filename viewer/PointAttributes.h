...
public:
	PointAttributes() :
		_attr(1, std::vector<float>(4, 1.0f)), _attr_size(1, 1), _attr_dim(1, 4), _curr_idx(0) {}
...
private:
	bool _unpack(const std::vector<char> & data, unsigned int expected_size) {
		...
		for (quint64 i = 0; i < num_attr; i++) {
			...
			if (attr_size[i] != expected_size && attr_size[i] != 1)
				return false;
	...
	void _compute_LOD(std::size_t attr_idx, const Octree & octree) {
		...
		if (false && _attr_dim[attr_idx] == 4) // disable LOD approximation of alpha transparency for now
	...
	bool _unpack_array(std::vector<T> & v, const char * & ptr, const char * const ptr_end) {
		if (ptr + sizeof(T) * v.size() > ptr_end)
			...
			std::copy((const T*)ptr, (const T*)(ptr + sizeof(T) * v.size()), v.begin());
			ptr += sizeof(T) * v.size();
			return true;
		}
	}
