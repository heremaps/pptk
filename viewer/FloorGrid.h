 ...
 *  Restricting to acute angles to ensure sector intersection results in at most one sector
 */
class Sector {
public:
	...
	Sector intersect (const Sector & other) const {
		...
		if (b->_start <= a->_end) {
			...
			return Sector(a->_start + 2.0f * pi(), std::min(a->_end + 2.0f * pi(), b->_end));
		} else
			return Sector();
	}
...
public:
	FloorGrid(QWindow * window, QOpenGLContext * context) :
		...
		_grid_floor_z(0.0f),
		_cell_size(1.0f), _line_weight(0.0f)
	{
...
private:
	...
	float visibleDistance(float cell_size, float projected_cell_size, const QtCamera & camera, float z_floor) {
	...
	bool computeHorizon(float & h_lo, float & h_hi, float cell_size, const QtCamera & camera, float z_floor) {
		...
		if (fabs(eye_floor_height) > d_max)
			return false;
		float theta_max = asin(eye_floor_height / d_max);	// TODO: handle eye_floor_height > d_max case
		...
		if (horizon_sector.empty())
			return false;
		h_lo = tan(-horizon_sector.getEnd() + theta);
		h_hi = tan(-horizon_sector.getStart() + theta);
		return true;
	}
	void computeCellSize(float & cell_size, float & line_weight, const QtCamera & camera, float z_floor) {
		...
		float d_0 = _window->height() / 2.0f / tan(alpha / 2.0f) / projected_cell_size;
	...
	void drawOrtho(const QtCamera & camera, float z_floor) {
		...
		float delta_image = delta_pixels / _window->height() / _window->devicePixelRatio() * 2.0f * t;
		float eps_x, eps_y;
		{
	...
	void drawPersp(const QtCamera & camera, float z_floor) {
		...
		float line_width = 1.2f / _window->height() / _window->devicePixelRatio() * 2.0f * t;
