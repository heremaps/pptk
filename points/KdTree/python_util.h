...
template <typename T>
bool extract_indices(std::vector<T> & indices,
...
template <typename T>
bool check_indices(const std::vector<T> & indices,
	const npy_intp max_index, const npy_intp item_index) {
	for (size_t i = 0; i < indices.size(); i++) {
		if ((npy_intp)indices[i] < -max_index || (npy_intp)indices[i] >= max_index) {
...
template <typename T>
void fix_negative_indices(std::vector<T> & indices,
	const npy_intp max_index) {
	for (size_t i = 0; i < indices.size(); i++) {
