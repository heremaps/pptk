	...
	if (PyArray_IsScalar(obj, Float)) {
		...
		*/
		...
		if (PyFloat_CheckExact(obj)) {
			...
			PyErr_Format(PyExc_TypeError,
				"Unsupported Python scalar type: %s",
				obj->ob_type->tp_name);
			return false;
		}
	} else {
...
bool extract_array(ListItem & item, PyObject * obj,
	Py_ssize_t item_idx, const char * operand_name = "") {
	...
	if (!PyArray_Check(obj)) {
		PyErr_Format(PyExc_TypeError, "Encountered non-array type: "
	...
	if (PyArray_TYPE(arr) != NPY_FLOAT32) {
		PyErr_Format(PyExc_TypeError, "Array dtype must be float32: "
			"item %d of %slist operand",
			(int)item_idx, operand_name);
		return false;
	}
	...
	if (ndim != 1 && ndim != 2) {
		PyErr_Format(PyExc_ValueError, "Array ndim neither 1 nor 2: "
			"item %d of %slist operand",
			(int)item_idx, operand_name);
		return false;
	}
...
bool extract_scalar_or_array(
	ListItem & item, PyObject * obj,
	npy_intp item_index, const char * operand_name) {
	if (PyArray_Check(obj)) {
		...
		PyErr_Format(PyExc_TypeError,
			"Unsupported item type: %s", obj->ob_type->tp_name);
		return false;
	}
	return true;
}
...
bool arrays_compatible(npy_intp & m_out, npy_intp & n_out, 
	npy_intp m_in1, npy_intp n_in1, npy_intp m_in2, npy_intp n_in2,
	Py_ssize_t item_idx) {
	if (	m_in1 != m_in2 && m_in1 != 1 && m_in2 != 1 ||
		...
		PyErr_Format(PyExc_ValueError,
			...
			(int)m_in1, (int)n_in1, (int)m_in2, (int)n_in2,
			(int)item_idx);
		return false;
	}
...
bool extract_operands_to_binop(Py_ssize_t & out_len,
	...
	if (x_len != y_len && x_len != 1 && y_len != 1) {
		PyErr_Format(PyExc_ValueError,
			"Incompatiable input list lengths %d,%d",
			(int)x_len, (int)y_len);
		return false;
	}
...
static PyObject * binary_op_single(PyObject * self, PyObject * args) {
	...
	if (!arrays_compatible(out_dims[0], out_dims[1],
		...
		return NULL;
	PyArrayObject * out_item = (PyArrayObject*)
		PyArray_EMPTY(2, out_dims, NPY_FLOAT32, 0);
...
static PyObject * binary_op(PyObject * self, PyObject * args) {
	...
	if (!extract_operands_to_binop(out_len,
		X, x_len, x_struct, Y, y_len, y_struct, args))
		return NULL;
	...
	for (Py_ssize_t i = 0; i < out_len; i++) {
		...
		npy_intp out_dims[2];
		if (!arrays_compatible(out_dims[0], out_dims[1],
			x_struct.m, x_struct.n, y_struct.m, y_struct.n, i))
			return NULL;
		PyArrayObject * out_item = (PyArrayObject*)
			PyArray_EMPTY(2, out_dims, NPY_FLOAT32, 0);
		float * out_data = (float*)PyArray_DATA(out_item);
...
static PyObject * reduction_op_single(
	...
	if (!PyArg_ParseTupleAndKeywords(
		args, kwdict, "O|O", keywords, &X, &axis_obj)) {
		PyErr_SetString(PyExc_TypeError, "Failed to parse arguments");
		return NULL;
	}
	int axis;
	if (axis_obj != Py_None) {
		...
		if (temp) {
			...
			PyErr_Format(PyExc_TypeError, "Type %s object passed "
				"as axis option", axis_obj->ob_type->tp_name);
			return NULL;
		}
	} else {
	...
	if (axis == -1) {
		...
		*out_data = reducer.finalize();
		...
		PyObject * out_item = PyArray_EMPTY(2, out_dims, TypeMap<ResultType>::type_num, 0);
...
template <typename T> struct reducer_mean {
	...
	float sum;
	npy_int64 count;
	void initialize() {sum = 0.0f; count = 0;}
	void accumulate(T v) {sum += v; count++;}
...
static PyObject * _sdot(PyObject * self, PyObject * args) {
	...
	if (x_struct.n != y_struct.m && !x_is_scalar && !y_is_scalar) {
		PyErr_Format(PyExc_ValueError,
			"Incompatible matrix sizes (%d, %d), (%d, %d)",
			(int)x_struct.m, (int)x_struct.n,
			(int)y_struct.m, (int)y_struct.n);
		return NULL;
	}
	npy_intp out_dims[2];
	if (x_is_scalar) {
	...
	if (x_is_scalar) {
		...
		for (npy_intp r = 0; r < x_struct.m; r++) {
			for (npy_intp c = 0; c < x_struct.n; c++)
				*out_data++ = 
					*(x_ptr + c * x_struct.col_stride) * y_val;
			x_ptr += x_struct.row_stride;
		}
	} else {
		const float * x_ptr = x_struct.data;
...
static PyObject * _transpose(PyObject * self, PyObject * args) {
	...
	if (!PyList_Check(X)) {
		PyErr_SetString(PyExc_TypeError,
			"Requires list as first argument");
		return NULL;
	}
	Py_ssize_t x_len = PyList_Size(X);
	if (x_len == 0) {
	...
	for (Py_ssize_t i = 0; i < out_len; i++) {
		ListItem x_struct;
		if (!extract_array(x_struct,
			PyList_GetItem(X, i), i, "indexee "))
			return NULL;
		MapTypeConst mat_x(x_struct.data, x_struct.m, x_struct.n,
			StrideType(x_struct.row_stride, x_struct.col_stride));
		npy_intp out_dims[2] = {x_struct.n, x_struct.m};
...
bool extract_integer(
	npy_intp & number, PyObject * item, npy_intp item_index) { 
...
bool extract_indices(vector<npy_intp> & indices,
	PyObject * obj, Py_ssize_t item_idx) {
	...
	if (PyList_Check(obj)) {
		...
		for (Py_ssize_t i = 0; i < len; i++) {
			npy_intp number;
			if (!extract_integer(number,
				PyList_GetItem(obj, i), item_idx))
				return NULL;
		...
		if (PyArray_TYPE(arr) == NPY_BOOL) {
			...
			if (PyArray_TYPE(arr) == NPY_INT64) {
				...
				PyErr_Format(PyExc_TypeError, "On %d-th item, "
...
bool check_indices(const vector<npy_intp> & indices,
	const npy_intp max_index, const npy_intp item_index) {
...
void slice_rows (ListItem & item_struct, PySliceObject * slice) {
	...
	PySlice_GetIndicesEx(slice, item_struct.m,
		&start, &stop, &step, &m_new);
...
void slice_cols (ListItem & item_struct, PySliceObject * slice) {
	...
	PySlice_GetIndicesEx(slice, item_struct.n,
		&start, &stop, &step, &n_new);
	item_struct.data += start * item_struct.col_stride;
	item_struct.col_stride *= step;
	item_struct.n = n_new;
}

void slice_rows_and_cols (ListItem & item_struct,
	PySliceObject * row_slice, PySliceObject * col_slice) {
...
static PyObject * _idx(PyObject * self, PyObject * args) {
	...
	if (!PyTuple_Check(I) || PyTuple_GET_SIZE(I) != 2) {
		PyErr_SetString(PyExc_TypeError,
			"Requires 2-tuple as second argument");
		return NULL;
	}
	...
	if (x_len == 1) {
		...
		if (index_type == 0)
			slice_rows_and_cols(x_struct,
				(PySliceObject*)Ir, (PySliceObject*)Ic);
	...
	if (index_type == 0) {
		...
		for (Py_ssize_t i = 0; i < out_len; i++) {
			...
			npy_intp out_dims[2] = 
				{x_struct.m, (npy_intp)indices.size()};
			PyArrayObject * out_item = (PyArrayObject*)
				PyArray_EMPTY(2, out_dims, NPY_FLOAT32, 0);
			const float * in_data = x_struct.data;
			float * out_data = (float*)PyArray_DATA(out_item);
		...
		for (Py_ssize_t i = 0; i < out_len; i++) {
			...
			npy_intp out_dims[2] = 
				{(npy_intp)indices.size(), x_struct.n};
			PyArrayObject * out_item = (PyArrayObject*)
				PyArray_EMPTY(2, out_dims, NPY_FLOAT32, 0);
			float * out_data = (float*)PyArray_DATA(out_item);
...
static PyObject * _seigh(PyObject * self, PyObject * args) {
	...
	PyArrayObject * eigvals = (PyArrayObject*)
		PyArray_EMPTY(1, out_dims, NPY_FLOAT32, 0);
	...
	PyArrayObject * eigvecs = (PyArrayObject*)
		PyArray_EMPTY(2, out_dims, NPY_FLOAT32, 0);
...
static PyMethodDef method_table[] = {
	...
	{"_sum", (PyCFunction)_sum, METH_KEYWORDS, "[sum(x1),...,sum(xn)]<-sum([x1,...,xn])"},
	...
	{"_mean", (PyCFunction)_mean, METH_KEYWORDS, "[mean(x1),...,mean(xn)]<-mean([x1,...,xn])"},
	...
	{"_stranspose", _stranspose, METH_VARARGS, "[x1.T,...,xn.T]<-[x1,...,xn].T"},
